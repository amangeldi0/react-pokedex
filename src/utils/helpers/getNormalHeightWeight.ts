import { getNormalHeightWeightReturn } from '@types';


const toLbs = (value: number): string => {
    const math = Math.floor((value * 2.2));
    return math.toString().split('').slice(0, math.toString().length - 1).join('');
};

const toFeet = (value: number): string => {
    const math = (value * 3.280839895);
    return  math.toString().split('').slice(0,math.toString().split('').indexOf('.') + 2).join('');

};


export const getNormalHeightWeight = (height: number, weight: number): getNormalHeightWeightReturn => {

    const weightLength: number = weight.toString().length;
    const heightLength: number = height.toString().length;

    const resultHeight: string = height.toString().split('').slice(0, heightLength - 1).join('');
    const resultWeight: string = weight.toString().split('').slice(0, weightLength - 1).join('');
